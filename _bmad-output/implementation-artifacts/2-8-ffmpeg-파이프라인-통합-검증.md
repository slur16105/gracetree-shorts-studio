...
- [ ] 작고 결정적인 script/voice/image/BGM/video/font fixture 세트를 고정한다. (AC: 1)
- [ ] 2.2~2.7 단계를 production orchestrator 경로로 실행하는 integration harness를 만든다. (AC: 1)
- [ ] ffprobe로 dimensions/fps/duration/streams를 검사하고 blackdetect/freezedetect 기준을 적용한다. (AC: 1)
- [ ] 단계별 wall time과 redacted command diagnostics를 attempt log에 기록한다. (AC: 1)
- [ ] 실패 단계가 terminal failed로 귀결되고 partial output이 final로 이동하지 않는지 검증한다. (AC: 1)
