...
- [x] `004_create_resources.sql`과 resource repository를 구현한다. (AC: 3, 4, 9) — 주: migration 003이 기 사용 중이므로 004 사용
- [x] 지원 resource type과 상태, resource DTO를 공유 계약으로 정의한다. (AC: 1~9)
- [x] file dialog→임시 복사→유형/가독성 검증→원자적 교체 service를 구현한다. (AC: 3, 4, 7)
- [x] 설정 dialog 내용을 Story 1.2 shell에 연결하고 행별 상태·교체·포커스 동작을 구현한다. (AC: 1~4, 8)
- [x] readiness와 BGM resolver에 `job override > valid default > missing` 우선순위를 구현한다. (AC: 2, 5, 6)
- [x] migration, 교체 rollback, 기존 완료 불변, modal accessibility 테스트를 추가한다. (AC: 1~9)
...
- Python: 004_create_resources.sql (UNIQUE/INDEX/CHECK 제약, 4개 타입 초기 시드), resource_repository, resource_service (MANAGED_ROOT_MISSING/SOURCE_UNREADABLE/UNSUPPORTED_FORMAT/COPY_FAILED, atomic backup-then-replace).
- IPC: register-resource-handlers.ts (get/update/select-file), resource-paths.ts, main/index.ts 등록.
- UI: SettingsDialog 4개 ResourceRow 완성, focus trap, getResources 호출; readiness.ts BGM fallback (per-job OR default_bgm) + commonResourcesReady gate.
...
- `engine/migrations/004_create_resources.sql`
- `engine/gracetree_engine/storage/resource_repository.py`
- `engine/gracetree_engine/inputs/resource_service.py`
- `engine/gracetree_engine/cli.py`
- `engine/tests/test_resource_repository.py`
- `engine/tests/test_resource_service.py`
...
- `apps/desktop/src/main/resources/resource-paths.ts`
- `apps/desktop/src/main/ipc/register-resource-handlers.ts`
- `apps/desktop/src/main/ipc/register-resource-handlers.test.ts`
...
- `apps/desktop/src/renderer/src/components/SettingsDialog.test.tsx`
