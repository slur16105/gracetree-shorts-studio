...
export interface JobDto {
  id: string
  publishDate: string
  status: 'draft' | 'running' | 'completed' | 'failed' | 'cancelled' | 'interrupted'
  title: string | null
  workPath: string
  resultPath: string
  createdAt: string
...
export type EngineCommand = CheckHealthCommand | GetOrCreateJobCommand
export type EngineEvent = HealthCheckedEvent | JobLoadedEvent
...
export function isGetOrCreateJobCommand(value: unknown): value is GetOrCreateJobCommand {
