...
from .aligner import Segment, normalize, lcs_ratio
...
def _current_rss_mb() -> float:
    """Return current process RSS in MB (process-lifetime high-water mark on macOS/Linux)."""
    try:
        import resource
...
def run_benchmark(
    ...
    samples = [BenchmarkSample(**s) for s in manifest_data["samples"]]
    ...
    for config in configs:
        for sample in samples:
            ...
            ref_normalized = normalize(sample.reference_text)
            ...
            peak_mem_delta = round(max(0.0, _current_rss_mb() - mem_before), 1)
